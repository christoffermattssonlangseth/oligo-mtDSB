...
adata.obs.transcript_counts.max()
...
genes_oligodendrocytes = [
    # Age 21
    "Hcrt", "Trh", "Pdyn", "Gfap",
    "Syt4", "Sesn2", "H2-D1", "Per1",
    "Hap1", "Epha5", "FlnA", "Fkbp5", "H2-K1",
    "Cbln2", "Vegfa", "Zbtb16", "Adarb1", "Atf5", "Jade2",
    "Gstp1", "Ddit4", "Dnajb1", "Meg3", "Ndn", "Eno2", "Hcn2",

    # Age 60
    "Nmu", "Cdkn1a", "Serpina3n", "Cd74",
    "Gfap", "Ccnd1", "Atf5", "Cemip2", "Gpr17", "9630013A20Rik", "Tnr",
    "Mt2", "Gpd1", "Fkbp5", "Cab39l", "Gad1", "Egr1", "H2-D1",
    "Corolc", "Hsp9"
]
...
expr_summary_sub[expr_summary_sub.gene.isin(genes_oligodendrocytes)]
keep_mask = expr_summary['expr_score'] >= 0.05


# In[462]:


expr_summary_filt = expr_summary[keep_mask]


# In[463]:


expr_summary_filt[expr_summary_filt.gene.isin(['Serpina3n'])]
...
for ct in gene_universe.keys():
    print(ct)
    print(len(gene_universe[ct]))
...
# example: combine your per-region tables into one df
# df_all = pd.concat([df_cortex, df_fiber, df_hypothalamus, ...], ignore_index=True)
...
def plot_region_signature_bars_by_age(
    ...
    return fig, axes
# usage:
# plot_region_signature_bars(df_all, top_n=5)
...
df_all_oligo = build_df_all_by_age(results_by_region, "Oligodendrocytes",)
df_all_micro = build_df_all_by_age(results_by_region, "Microglia")
# etc.
...
# plot side-by-side P21 vs P60
fig, axes = plot_region_signature_bars_by_age(
    df_all_oligo,
    top_n_genes_per_region=3,
    figsize=(14,6),
    xlim=None,        # or e.g. (0,6) if you want fixed scale
    sort_regions_by="max_lfc"
)
...
ct = "Astrocytes"   # or "Oligodendrocytes", etc.

df_all = []  # list to collect per-region frames

for region, payload in results_by_region[ct].items():
    for age in ["21", "60"]:
        if "simple_effects" not in payload or age not in payload["simple_effects"]:
            continue
        df = payload["simple_effects"][age].sort_values("log2FoldChange", ascending=False)
        df["region"] = region
        df["cell_type"] = ct
        df["age"] = age
...
df_all = df_all[df_all.baseMean > 200]

# tidy up types
df_all["baseMean"] = df_all["baseMean"].astype(float)
df_all["log2FoldChange"] = df_all["log2FoldChange"].astype(float)
df_all["pvalue"] = pd.to_numeric(df_all["pvalue"], errors="coerce")

print(df_all.head(10))
print(df_all.shape)
plot_region_bubbles(df_all)
plot_region_signature_bars(df_all)


# In[484]:


results_by_region['Oligodendrocytes']['Fiber tracts']['simple_effects']['60'].sort_values(by = 'log2FoldChange', ascending = False).head(10)


# In[482]:


results_by_region['Oligodendrocytes']['Thalamus']['simple_effects']['60'].sort_values(by = 'log2FoldChange', ascending = False).head(10)
